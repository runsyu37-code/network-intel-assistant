...
>
> ⚠️ **Note:** Original draft written by frontend team — several tech stack details were incorrect. Corrected below.

---

## Your Role in This Review

You are the **backend builder**. You designed and built the ASP.NET Framework 4.8 REST API across Phase 7–13, then coordinated with the frontend team via spec documents in rounds R1–R4. The reviewer has read `REVIEW_BRIEF.md` and will ask you to walk them through the work, justify decisions, and explain edge cases.
...
| Phase / Round | What Was Delivered |
|---|---|
| Phase 7–9 | Auth (JWT), full CRUD for all device types, rate limiting (10 fail → 15-min lockout), audit log |
| Phase 10 | RBAC enforcement — RequireRole filter on every endpoint, 43/43 security gate PASS |
| Phase 11–13 | Adversarial review, backlog closure, reflection security gate |
| F9 R1–R2 | Frontend coordination — confirmed all existing endpoints match frontend spec, no new code needed |
| F9 R3 | Rack endpoints — GET /api/racks (enriched JOIN + computed fields), GET /api/racks/{rackId} (devices[] + alerts[]) |
| F9 R4 | PATCH /api/cameras/{id}/position updated to 0–100 range; cameraCount + nvrCount added to BuildingTreeDto |

---

## Key Technical Decisions — and Why

### 1. ASP.NET Web API (.NET Framework 4.8), not Express or FastAPI
**Decision:** Use ASP.NET Web API with C# for all controllers.
**Why:** The organisation already uses .NET infrastructure. Strong typing prevents entire classes of runtime bugs. The deployment environment is Windows Server IIS — .NET Framework 4.8 is the natural fit and requires zero runtime installation on existing servers.
**Data access:** Raw ADO.NET (`SqlConnection` / `SqlCommand`) — no ORM. Direct SQL gives full control over query shape, especially important for the computed-field aggregations in rack endpoints.
**Trade-off accepted:** More verbose than Node/Python. Acceptable given team familiarity and operational requirements.
...
**Production:** Replace with the actual intranet URL in `Web.config` (or via Web.config transform for multi-environment deploy).
...
user    → read + write (GET + POST device CRUD)
admin   → full access (all endpoints + Users CRUD)
```

Users endpoint (`/api/users/*`) is admin-only.
`PATCH /api/cameras/{id}/position` is admin-only.
All other write endpoints (device CRUD) require admin or user.
GET endpoints are accessible to all authenticated roles.
...
- `App_Start/WebApiConfig.cs` — routing + CORS policy setup
- `Filters/JwtAuthFilter.cs` — JWT middleware (custom IAuthorizationFilter)
- `Web.config.template` — connection string + JWT secret structure (Web.config is gitignored)
