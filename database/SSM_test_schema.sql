-- =============================================================================
-- SSM v1.0 — TEST Schema (SSM_TEST_DB)
-- Date   : 2026-05-21
-- Purpose: Drop + Recreate SSM_TEST_DB พร้อมตารางเปล่า
--          Column order ตรงกับ template_v4_empty.xlsx เพื่อ copy-paste ได้เลย
--
-- วิธีใช้: เปิดไฟล์นี้ใน SSMS แล้วกด F5 รันทีเดียว
--
-- =============================================================================
...
IF EXISTS (SELECT name FROM sys.databases WHERE name = N'SSM_TEST_DB')
BEGIN
...
-- Column order ตรงกับ Excel ทีละ sheet (ไม่รวม # / Image Path / Created / Updated)
-- Auto-fill columns (image_data, image_type, created_at, updated_at) อยู่ท้ายสุด
-- =============================================================================
...
-- Excel col order: Site_ID, Building_ID, Floor_ID, Room_ID, Rack_ID, SW_ID,
--   U Position, U Sub-pos, Device Name, Switch Type, Brand, Model, S/N, MAC,
--   OS/Firmware, VLAN, IP Address, Total Ports, PoE Ports, PoE Budget(W),
--   PoE Used(W), Uplink Port, Status, Fail Count, Note
CREATE TABLE poe_switches (
    ...
    -- ── DB only (not in Excel template_v4) ──
...
-- Excel col order: Site_ID, Building_ID, Floor_ID, Room_ID, Rack_ID, NVR_ID,
--   U Position, U Sub-pos, U-Size, Device Name, Brand, Model, S/N, MAC,
--   OS/Firmware, VLAN, IP(Internet), IP(CCTV), Total CH, Active CH, HDD Total(TB),
--   Rec Resolution, Retention(days), Record Status, Status, Fail Count, Note
CREATE TABLE nvrs (
    -- ── Excel columns (paste order) ──
    Site_ID         NVARCHAR(10)               NOT NULL,  -- col B
    Building_ID     NVARCHAR(10)               NOT NULL,  -- col C
    ...
    serial_no       NVARCHAR(100)              NULL,      -- col N  Serial No (S/N)
    mac_address     NVARCHAR(20)               NULL,      -- col O  MAC Address
    os_version      NVARCHAR(50)               NULL,      -- col P  OS / Firmware (firmware)
    vlan_id         INT                        NULL,      -- col Q  VLAN
...
--   ส่วน 1: cols B–R (17 cols): Site_ID ถึง Switch Port
--   ส่วน 2: cols U–W (3 cols): Status, Fail Count, Note
--   (ข้าม col S = NVR Device Name และ col T = NVR Channel เพราะซ้ำกับ col F, G)
-- col E ID(PK) → NVR_CH,  col F NVR → NVR_ID,  col G CH(Port) → nvr_channel
CREATE TABLE cameras (
    -- ── IDENTITY PK (auto, SSMS จะข้ามให้อัตโนมัติ) ──
    id              INT          IDENTITY(1,1) NOT NULL,
    -- ── Excel columns ส่วน 1 (paste cols B–R) ──
    Site_ID         NVARCHAR(10)               NOT NULL,  -- col B
    Building_ID     NVARCHAR(10)               NOT NULL,  -- col C
    Floor_ID        NVARCHAR(10)               NOT NULL,  -- col D
    NVR_CH          NVARCHAR(30)               NULL,      -- col E  ID(PK) — NVR+CH key
    NVR_ID          NVARCHAR(20)               NULL,      -- col F  NVR (FK to nvrs)
    ...
    -- ── Excel columns ส่วน 2 (paste cols U–W หลังจาก paste ส่วน 1 แล้ว) ──
...
CREATE TABLE sync_logs (
    ...
    fields_updated NVARCHAR(MAX)                        NULL,
...
-- =============================================================================
-- SECTION 6 : SEED USERS (สำหรับ login เข้าเว็บ — ไม่ใช่ข้อมูล survey)
-- ⚠️  เปลี่ยน bcrypt hash ก่อน production
--     admin    → Admin@SSM1
--     ssm_user → User@SSM1
-- =============================================================================
...
-- =============================================================================
-- END OF SSM_test_schema.sql
-- 13 tables, 5 views, 25+ indexes — ตารางข้อมูลทั้งหมดว่างเปล่า
-- =============================================================================
