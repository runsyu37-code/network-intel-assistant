...
| # | Question | Answer |
|---|---|---|
| 1 | Is `PATCH /api/cameras/{id}/position` protected? | ✅ Yes — `[RequireRole("admin")]` at `camerasController.cs:271`. Viewer cannot call it directly. |
| 2 | Are read endpoints role-gated? | ✅ Yes — `GET /api/cameras`, `/nvrs`, `/poe-switches`, `/users` are admin-only. `/racks` and `/rooms` are admin+user. Matches ROLE_MATRIX. |
| 3 | Is `res.role` from DB or request body? | ✅ From DB — `authController.cs:85` reads `role` from `users` table. `LoginRequest` has no role field. Cannot be injected. |

**Conclusion:** Backend enforcement is complete. Frontend fixes are self-contained — backend has no additional work.

---

## ⚠️ Flag from Backend Team — Verify Before Implementing F2-1

Backend reports that the `SELECT` statement for `GET /api/cameras` may **not** include `position_x` and `position_y` fields. If confirmed, `mapApiCamera` will always receive `null` for both — meaning:
- Camera pins always fall back to the 4-column grid layout regardless of saved positions
- The drag-save PATCH writes data that is never read back correctly

**Action required before fixing F2-1:** Fire a real `GET /api/cameras` call and inspect the response JSON. If `position_x`/`position_y` are missing, raise with backend to add them to the SELECT. This may be a backend bug, not a frontend bug.
...
| Fix | Phase | Status |
|---|---|---|
| F1-1: RouteGuard component + DashboardPage routes | 1 | ✅ Done (commit ee47c54) |
| F1-1: NotAuthorizedPage (403) | 1 | ✅ Done (commit ee47c54) |
| F1-2: FloorPlanPage edit mode role guard | 1 | ✅ Done (commit ee47c54) |
| F2-1: Drag save revert on failure + toast | 2 | ✅ Done (commit b3d83f7) |
| F2-2: Remove fallback data — CamerasPage | 2 | ✅ Done (commit 1c648a9) |
| F2-2: Remove fallback data — FloorPlanPage | 2 | ✅ Done (inline — no FALLBACK array was present) |
| F2-2: Remove fallback data — NVRsPage, SwitchesPage, RacksListPage | 2 | ✅ Done (commit 1c648a9) |
| F2-3: Fix site filter (dynamic dropdown from API data) | 2 | ✅ Done (commit 1c648a9) |
| F3-1: Handle 403 in Axios interceptor | 3 | ✅ Done |
| F3-2: Floor plan images — Option A documented (intranet accepted) | 3 | ✅ Done |
