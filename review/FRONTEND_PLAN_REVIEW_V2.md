...
> **Version:** 2.0 — incorporates Ran's feedback from Round 1 and Round 2
...
This V2 incorporates Ran's pushback and clarifications from 2 rounds of feedback:
...
| 14. Action items | Original 8 items | **Added: schema changes for camera position + floor_plans table** | Needed before Floor Plan layer can work |
...
10. [RACK_POSITION.md + Camera Position Spec (REVISED)](#10-rack_positionmd--camera-position-spec-revised)
...
### โดยรวม
Ran ทำงานในระดับที่ **เกินคาดสำหรับ intern** — มี engineering mindset, documentation ครบ, test coverage จริง, และ wireframe ที่ออกแบบมีความคิด UX ที่ดี การพูดคุย review 2 รอบที่ผ่านมา Ran มี **product thinking** ที่ดี ไม่ใช่แค่ technical thinking — push back ด้วยเหตุผลในจุดที่ควร push back

### แต่ความเป็นจริงต้องพูดตรงๆ
Scope ของ Phase 7 (รวม Quick Add + RBAC + drag-drop floor plan + realtime polling) ยัง **เกินกำลัง 1 คน × 6 สัปดาห์** ผมยังยืนยันว่าต้องตัด feature บางอย่าง — แต่ตัดน้อยกว่า V1
...
- ดู Section 10 สำหรับ floor_plans table spec
...
## 10. RACK_POSITION.md + Camera Position Spec (REVISED)
...
CREATE TABLE floor_plans (
    ...
    image_path     NVARCHAR(500) NOT NULL,  
        -- e.g. "/uploads/floor_plans/site1_bldA_floor3_v2.png"
...
#### Floor Plan Upload — Manual Workflow (Ran's decision)

**Decision:** IT/admin copy image file ไป server (FTP/manual), แล้ว register path ผ่าน UI

**Trade-off:**
- ✅ Simple, ไม่ต้อง handle multipart upload, no file size limit
- ⚠️ Admin ต้อง access server file system
- ⚠️ Path validation — UI ต้องตรวจว่า file มีอยู่จริงก่อน save (ไม่ใช่ admin พิมพ์ผิด save ผ่าน)

**HOW (mitigation):**

**1. Backend endpoint สำหรับ validate path:**
```csharp
[HttpPost("floor-plans/validate-path")]
public ApiResponse<FloorPlanValidationDto> ValidatePath(string path) {
    // ตรวจว่า:
    // 1. Path อยู่ภายใต้ /uploads/floor_plans/ (กัน path traversal)
    // 2. File มีอยู่จริง
    // 3. Extension เป็น image (.png, .jpg, .svg)
    // 4. อ่าน image dimensions ได้
    
    if (!path.StartsWith("/uploads/floor_plans/"))
        return Error("Path must be under /uploads/floor_plans/");
    
    var fullPath = Path.Combine(_webRoot, path.TrimStart('/'));
    if (!File.Exists(fullPath))
        return Error("File not found at specified path");
    
    var (width, height) = ImageHelper.GetDimensions(fullPath);
    return Ok(new { isValid = true, imageWidth = width, imageHeight = height });
}
```

**2. UI workflow:**
```
1. Admin upload file ไป server ผ่าน FTP/SMB share
2. เปิด Floor Plan management page
3. กดปุ่ม "Register Floor Plan"
4. กรอก path: /uploads/floor_plans/site1_bldA_floor3.png
5. UI เรียก validate-path endpoint
6. Backend ตรวจ → คืน { valid, width, height }
7. Admin เห็น preview ของรูป → กด Confirm
8. Save record ใน floor_plans table
```
...
**4. Storage:** File system ของ server (ตาม Ran's decision)
...
#### [P1] NEW — Floor Plan Endpoints (เพิ่มใน V2)

ต้องมี endpoint จัดการ floor_plans table:

```
GET    /api/floor-plans?floorId=X        → list plans ของ floor (รวม inactive)
GET    /api/floor-plans/{id}              → get specific plan
GET    /api/floors/{id}/active-plan       → get active plan ของ floor
POST   /api/floor-plans                   → register new plan (admin only)
POST   /api/floor-plans/validate-path     → ตรวจ path ก่อน register
PUT    /api/floor-plans/{id}/activate     → switch active plan version
DELETE /api/floor-plans/{id}              → soft delete (set is_active=0)
```

#### [P1] NEW — Camera Position Endpoint (เพิ่มใน V2)
...
| P0 | **Floor plan endpoints (CRUD + validate-path)** | 1 วัน |
...
**Total estimate: ~7-9 วันทำงาน ก่อนแตะ React** (เพิ่มจาก V1 = 5-7 วัน)
...
- [ ] **NEW: Schema change — camera position + floor_plans table**
- [ ] **NEW: Floor plan endpoints + validate-path**
- [ ] **NEW: Camera position PATCH endpoint**
- [ ] Stress mock data (1000+ rows)

**Estimate: 7-9 วัน** (เพิ่มจาก V1 เพราะมี schema change + floor plan endpoints)
...
#### Week 4 — Floor Plan + Rack + Quick Add (Camera)
- [ ] Floor Plan page (Konva.js)
  - ✅ View mode
  - ✅ Edit mode (drag camera)
  - ✅ Add/Delete camera
  - ✅ position_x/y save as percentage
- [ ] Floor plan registration UI (admin: register path)
- [ ] Room View (rack list)
...
|---|---|---|---|
| 1 | **NEW: ALTER TABLE cameras add position_x, position_y, position_set_at, position_set_by** | P0 | 30 นาที |
| 2 | **NEW: CREATE TABLE floor_plans + constraints + indexes** | P0 | 1 ชม. |
| 3 | **NEW: Update mock_data.sql to include sample positions** | P0 | 1 ชม. |
| 4 | **NEW: Update Excel template + SCHEMA version cell** | P0 | 1 ชม. |
| 5 | เพิ่ม CORS ใน C# API | P0 | 1 ชม. |
| 6 | เพิ่ม Auth endpoints + JWT | P0 | 1-2 วัน |
| 7 | สร้าง aggregate endpoints (tree, status, breadcrumb) | P0 | 1 วัน |
| 8 | **NEW: Floor plan endpoints + validate-path** | P0 | 1 วัน |
| 9 | **NEW: Camera position PATCH endpoint** | P0 | 2 ชม. |
| 10 | Setup Swagger | P1 | 30 นาที |
| 11 | Rename endpoints, ลบ `Get` prefix | P1 | 30 นาที |
| 12 | Generate stress mock data | P1 | 2 ชม. |
| 13 | Document deprecation ใน ssm_import.py | P2 | 15 นาที |

### Decisions Confirmed (Round 2)
...
| 5 | **NEW: `database/FLOOR_PLAN_SPEC.md`** | Camera position + floor_plans table spec |
| 6 | **NEW: `docs/workflow/FLOOR_PLAN_UPLOAD.md`** | Manual file copy procedure for admins |
...
6. **ถามคำถามที่ถูก** — "pixel vs percentage" คือคำถามที่ senior engineer ถาม
...
**Demo message:** "ระบบ end-to-end ครบ loop จาก DB → API → UI พร้อม auth + RBAC, device monitoring realtime, drag-drop floor plan management, และ Quick Add workflow สำหรับ technician"
...
**Week 0 ใน V2 = 7-9 วันแทน 5-7 วัน** เพราะเพิ่ม:
- Schema change (camera position + floor_plans table)
- Floor plan endpoints (CRUD + validate-path)
- Camera position PATCH endpoint

แปลว่า:
- ถ้า Ran มีเวลาทำงาน 8 ชั่วโมง/วัน เต็ม → ทันได้ (~9 วัน = 1.5 สัปดาห์)
- ถ้ามีอย่างอื่นแทรก (เรียน, ฝึกงานอื่น) → อาจกินเวลา 2-2.5 สัปดาห์
- จาก 6 สัปดาห์ deadline → เหลือ React 3.5-4 สัปดาห์เท่านั้น

**คำแนะนำ:** ถ้า Week 0 ลากเกิน 2 สัปดาห์ → ต้องตัด scope MVP เพิ่ม (เช่น defer Quick Add → Phase 7.5)
...
| Camera Position Spec (NEW) | ✅ Specified |
| Backend Changes | ✅ Reviewed (expanded) |
...
**Total findings:**
- P0: 11 (เพิ่มจาก V1 = 8) — เพิ่มจาก schema change + floor plan endpoints
- P1: 14 (เพิ่มจาก V1 = 12)
- P2: 11
- P3: 2

**Next steps:**
1. Ran รัน schema change scripts (camera position + floor_plans table)
2. ผมพร้อมช่วยลงรายละเอียดเฉพาะส่วนที่ Ran เลือก:
   - Backend code skeleton (JWT auth, aggregate endpoints, floor plan controller)
